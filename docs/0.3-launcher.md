...
## Arguments
  ...
  - **make-flags**="flags (e.g. `threads=t`, `no_ht=0`, `debug=1`, `wait_passive=1`, and/or `energy=1`) to be passed to the `make`, separated by space, and  double quoted" (default empty)
  ...
  - **mpiexec-args="args"** (args for mpiexec, defalut: "--report-bindings -n ${SLURM_NNODES} -npernode 1 -oversubscribe")
